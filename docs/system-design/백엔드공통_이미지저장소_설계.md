...
| 조건 | 값 | 영향 |
|------|-----|------|
| 앱 위치 | **사설 서브넷** | 2026-09-23 이전한다. S3 는 게이트웨이 엔드포인트로 직행하므로 NAT 요금을 안 탄다 |
| NAT Gateway | 2대 (AZ 당 하나) | ~~되돌리지 않는다~~ 고 적었으나 2026-09-23 에 뒤집혔다 (INF-09). 보안 그룹 단일 방어선의 한계 |
| 월 예산 | 약 140 ~ 181 USD | 이미지 저장소가 큰 비중을 차지하면 안 된다 |
