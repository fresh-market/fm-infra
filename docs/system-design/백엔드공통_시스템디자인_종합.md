...
graph TB
    ...
    subgraph aws["AWS ap-northeast-2"]
        ...
        subgraph vpc["VPC 10.0.0.0/16"]
            ...
            subgraph pub["퍼블릭 서브넷 10.0.1.0/24 AZ-a, 10.0.2.0/24 AZ-c"]
                NATA["NAT Gateway AZ-a<br/>private-a 의 출구"]
                NATC["NAT Gateway AZ-c<br/>private-c 의 출구"]
                LTI["load-test m7i-flex.large<br/>시험 때만 기동"]
            end

            subgraph pri["프라이빗 서브넷 10.0.11.0/24 AZ-a, 10.0.12.0/24 AZ-c"]
                APP1["app-1 t3.small AZ-a<br/>Docker Compose + systemd<br/>ASG min 2"]
                APP2["app-2 t3.small AZ-c<br/>상시 가동<br/>AZ 를 갈라 둔다"]
                CPN["coupon x3 t3.small<br/>선착순 전용 ASG<br/>이벤트 때만 desired 3"]
                MON["monitoring t4g.small<br/>Prometheus, Grafana<br/>Loki, Alertmanager"]
                BAT["batch t3.small<br/>스케줄러 전용<br/>ASG 밖, 인바운드 없음"]
                RDS1[("RDS MySQL 8.4.11<br/>db.t4g.micro<br/>PITR 보존 7일")]
                RDS2[("standby<br/>읽기 불가, 페일오버 전용")]
    ...
    %% 굵은 테두리는 지금 이중화가 없는 것이다.
    %% 굵은 테두리는 지금 이중화가 없는 것이다. 모니터링 한 대만 남았다.
    %% 앱은 min 2 로 AZ 를 갈랐고(2026-09-23), NAT 도 AZ 당 하나로 갈랐으며,
    %% 캐시는 2노드 자동 페일오버다(INF-37).
...
1. **앱은 트래픽에 따라 2~3 대로 움직인다** (2026-08-29 오토스케일링 전환, 2026-09-23 하한 상향). 한때 하한이 1이라 한산한 구간에서 앱이 단일 장애점이었다. ASG 가 죽은 인스턴스를 다시 만들지만 기동에 4~6분이 걸리고 그동안 서비스가 멈춘다. **자가 치유는 이중화가 아니다.**
...
4. **NAT Gateway 가 AZ 당 하나다.** 앱과 배치와 모니터링은 사설 서브넷에 있고 나갈 때만 NAT 를 거친다. ALB 와 부하 생성기만 퍼블릭에 남는다. 사설 라우팅 테이블도 AZ 별로 갈라 각자 자기 NAT 를 본다. 한 대로 뒀을 때는 그 AZ 가 죽으면 양쪽이 다 못 나갔다 (2026-09-23 이중화).
...
7. **관측은 자체 호스팅이며 알림 경로가 둘이다.** 관리형 대비 약 6배 저렴하고 스크랩 간격을 좁혀도 비용이 늘지 않는다. 다만 모니터링 인스턴스가 죽으면 감지 수단이 사라지므로, 핵심 알람만 CloudWatch 에 이중으로 걸어 두 번째 경로를 만든다.
...
퍼블릭에 남는 것은 셋뿐이다. 인터넷을 받는 ALB, 나가는 길인 NAT, 그리고 시험 도구다.
**사설 라우팅 테이블이 AZ 별로 갈려 각자 자기 AZ 의 NAT 를 본다.** 하나로 공유하면 NAT 를
둘로 늘려도 의미가 없다. 그 테이블이 가리키는 NAT 의 AZ 가 죽으면 반대편도 같이 못 나간다.
부하 생성기를 사설로 옮기지 않은 것은, 인터넷 대면 ALB 를 부를 때 NAT 를 왕복해
요금과 측정 변수가 함께 늘기 때문이다.
```
...
앱과 배치와 모니터링은 사설 서브넷에 있다. 퍼블릭에 남는 것은 ALB 와 NAT 게이트웨이, 그리고 시험 시간에만 뜨는 부하 생성기다.

한때는 반대였다. NAT 비용(월 40 USD 이상)이 전체 예산의 4분의 1이라 앱을 퍼블릭 서브넷에 두고 보안 그룹만으로 막았다(INF-09). 그 판단을 2026-09-23 에 뒤집었다. **보안 그룹이 유일한 방어선이면 규칙 하나를 실수로 `0.0.0.0/0` 으로 여는 순간 인터넷에 노출된다.** 사설 서브넷은 경로 자체가 없어 그 실수를 무르게 한다.

보안은 이제 두 겹이다. 밖에서 오는 경로가 없는 것이 1차이고 보안 그룹이 2차다. 인스턴스는 퍼블릭 IP 를 갖지 않으며, 나가는 길은 NAT 뿐이고, 운영 접근은 22번 포트가 아니라 SSM Session Manager 로 처리한다.

**NAT 는 AZ 당 하나다.** 사설 라우팅 테이블을 AZ 별로 갈라 각자 자기 NAT 를 보게 했다. 하나로 공유하면 NAT 를 둘로 늘려도 의미가 없다. 그 테이블이 가리키는 NAT 의 AZ 가 죽으면 반대편도 같이 못 나가기 때문이다.
...
| 부하 생성 -> ALB | 443 |
| 앱, 배치, 모니터링 -> ECR, SSM, 카카오 | NAT 경유 아웃바운드. S3 는 게이트웨이 엔드포인트 |
...
**아웃바운드는 다섯 모두 전체 허용이다.** ECR 이미지 pull, SSM Parameter Store 조회, 패키지 설치, 카카오 API 호출이 나간다. 사설 서브넷의 인스턴스는 NAT 를 거치고, S3 는 게이트웨이 엔드포인트로 빠진다. ECR 레이어의 실체가 S3 라 그 엔드포인트가 NAT 데이터 처리 요금의 상당 부분을 덜어 낸다. RDS 와 캐시는 같은 사설 서브넷에 있지만 나갈 일이 없다.
...
| 구조 | 관리형은 스크랩 간격을 좁힐수록 비용이 늘어 장애 관측과 상충 |

장애 검증에는 5~15초 간격이 필요하다. 60초 간격으로는 30초짜리 사건을 두 점으로밖에 볼 수 없다.
...
| 부하 생성 | m7i-flex.large, 약 40시간 | 4.71 USD |
