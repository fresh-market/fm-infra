...
graph TB
    ...
    %% 앱은 desired 1 구간에서만 그렇고(확장하면 해소), 모니터링과 NAT 는 상시 1대다.
    %% 캐시는 2노드 자동 페일오버라(INF-37) 여기 들지 않는다.
    classDef spof stroke:#A8481F,stroke-width:2px;
    class APP1,MON,NAT spof;
...
4. **NAT Gateway 가 한 대다.** 앱과 배치와 모니터링은 사설 서브넷에 있고 나갈 때만 NAT 를 거친다. ALB 와 부하 생성기만 퍼블릭에 남는다. AZ 당 하나가 운영 표준이지만 두 배 값이라 한 대로 둔다. 그 AZ 가 죽으면 양쪽 사설 서브넷이 다 못 나간다.
...
**NAT 는 한 대뿐이다.** AZ 당 하나가 운영 표준이지만 두 배 값이라 한 대로 둔다. 대신 그 AZ 가 죽으면 양쪽 사설 서브넷이 다 못 나가고, 카카오 로그인과 이미지 pull 이 함께 멈춘다.
