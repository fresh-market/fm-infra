/*
 * 앱은 사설 서브넷에 있다. 밖에서 오는 경로가 없는 것이 1차 방어이고, 보안 그룹이 2차다.
 * 그래서 인바운드는 CIDR 이 아니라 보안 그룹 참조로만 연다. 인터넷을 받는 ALB 만 예외다.
 *
 * 두 겹으로 둔 이유는 서로의 실수를 덮기 위해서다. 규칙 하나를 0.0.0.0/0 으로 잘못 열어도
 * 서브넷에 경로가 없어 닿지 않고, 서브넷을 잘못 옮겨도 보안 그룹이 남는다.
...
/*
 * 아웃바운드는 전부 열어 둔다.
 * ECR 이미지 pull, SSM Parameter Store 조회, 패키지 설치, 카카오 API 호출이 나간다.
 * 사설 서브넷의 인스턴스는 NAT 를 거치고, S3 는 게이트웨이 엔드포인트로 빠진다.
 * ECR 레이어의 실체가 S3 라 그 엔드포인트가 NAT 데이터 처리 요금의 상당 부분을 덜어 낸다.
 */
